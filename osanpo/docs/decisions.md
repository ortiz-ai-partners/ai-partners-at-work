# 決めたこと一覧（忘れた時はここだけ見る）

最終更新: 2026-09-25

## いまの目的

**Physical AI 応用1講座の最終課題（締切 2026-11-02 AM10:00）の提出物にする。**
要件・日程・加点戦略は [pai-final.md](pai-final.md)。PARC2026は本選1で敗退したため、11/1まで全力をここに使える。

## ハード

| 何 | 決定 |
|---|---|
| 目・耳・口・顔 | StackChan（M5Stack公式キット、中身はCoreS3相当、カメラ0.3MP、700mAh） |
| 頭脳（受信サーバ） | タクミさんにもらったミニPC「A5」。**Ryzen 5 7430U / 32GB DDR4-3200 / 477GB SSD**、Windows 11、USB-C 19W給電、ユーザー名 airae |
| ご自宅LLMの保険 | 32GBあるので MoE 30B-A3B 級（約18GB）や 7〜8B 級はCPUで実用速度。密な27Bは載るが遅い（2〜3 tok/s）。GPU箱は「足りなくなってから」 |
| 外で動かす時の電源 | モバイルバッテリー。PD **30W以上**推奨（19Wぎりぎりだと落ちる） |
| 通信 | iPhoneのテザリング |
| Xiao ESP32S3 Sense | **買わない**（StackChanで全部足りる） |
| 今すぐの止血 | StackChanの電源を切る（出荷時アプリは声と写真をクラウドへ送る）。Unbind & Reset は早めに |

## ソフト・設定

| 何 | 決定 |
|---|---|
| 受信ポート | **5072** |
| 合言葉 | `osanpo/windows/osanpo.env` の `OSANPO_TOKEN`（gitに入らない） |
| 頭脳 | **Claude**（Claude Code経由、サブスクの範囲で従量課金なし）。`OSANPO_BRAIN=openai` の経路も残してあるが使わない |
| 散歩に出る人格 | **新しい子（おさんぽの子）**。ヴェルティは参謀として日記を読む側。`OSANPO_PERSONA` 既定 `osanpo` |
| その子の名前 | **しばらく交流してから、本人に決めてもらう** |
| その子の役割・性格 | おさんぽのおとも。好奇心旺盛で、見て学ぶ子 |
| 想像の扱い | 見えないものを想像してよい。ただし「たぶん」「想像だけど」を必ず付ける |
| その子の記憶 | 三層。散歩の中は1つの会話を再開し続ける（写真も会話も全部）／散歩をまたぐ直近8発言／`memory.md`（散歩の終わりに自動内省） |
| 知らない顔を覚えるか | **作った**（ゆうころの設計）。近く・正面・大きい顔だけ数え、5回以上かつ2日以上で括弧書きで聞く。名前を教えたら登録、「だめ」で消す。30日で自動消去。散歩はゆうころ本人なので会うのは知人と家族の想定（子どもを登録する時だけ親に一言） |
| その子の声 | **ゆうころの声をAIで学習して当てる**（フェーズ3。道具はその時に選ぶ） |
| 日記 | `diary.jsonl`（機械用、将来の教材、消さない）＋ `diary.log`（人間用） |
| 家族の顔認識 | ミニPC内でOpenCV照合。Claudeには名前だけ渡す。顔データは外に出さない |
| 顔登録の前に | たくみさんに了解を取る |

## Windows 11 初期設定

| 画面 | 選ぶもの |
|---|---|
| 個人用 / 職場または学校用 | **個人用** |
| Microsoftアカウント | **いつものアカウント** |
| PIN | 作ってよい |
| OneDriveのバックアップ | **オフ**（顔写真やリポジトリをクラウドに出さないため） |
| 位置情報 | **オン**＋「デバイスの検索」オン（背負って出る機械なので紛失対策。GPSは無くWi-Fi推定、ネットに繋がった時だけ報告） |
| 診断データ・広告ID・アプリごとの位置情報許可 | オフでよい |
| スマホ連携・365・Game Pass等の案内 | スキップ |
| 後で | 自動ログオン（netplwiz）、スリープ禁止、リモートデスクトップON |

## StackChan のファームウェア

| 何 | 決定 |
|---|---|
| 出荷時アプリの正体 | 小智（Xiaozhi）ベース。声・写真・会話が **小智のクラウド（中国）** に送られる。頭脳は Qwen / DeepSeek / DouBao |
| 出荷時アプリを使うか | **使わない**（画像を中国に送るのが不安、が決め手。「何が見える？」の試用もしない） |
| 本命 | **stackchan-mcp**（第三者のMIT製ファーム＋Pythonゲートウェイ）。Claude Code から take_photo / say / listen / move_head / set_avatar を道具として呼ぶ |
| うちの自作スケッチ | **不採用**（stackchan-mcp が同じものを持っている） |
| 書き込む前に | ① iPhoneアプリで Unbind & Reset ② esptool でフラッシュ16MB吸い出し（`firmware/README.md`）③ 公式の戻し方は M5Burner の StackChan |
| stackchan-mcp ファームの更新確認先 | 既定は小智のクラウド。**カスタムOTA URL を `http://ミニPC:5072/xiaozhi/ota/` にして家の中で完結させる**（server.py が返答）。2026-09-17 に一度、空のまま起動して本体の機種名・MAC・端末IDが xiaozhi.me に渡った。声・写真は渡っていない |
| データの行き先 | 写真とその子の言葉 → Claude（Anthropic、米国）のみ。声の文字起こし(faster-whisper)・合成(VOICEVOX)・顔照合・日記・記憶は **ミニPCの中**。ElevenLabs で声クローンを使う時だけ文字が米国へ |
| 確認すること | Claude 側で写真が学習に使われるかの設定項目（契約と設定に従う。未確認） |

## ドメイン・外から届く経路

| 何 | 決定 |
|---|---|
| 方式 | **Cloudflare Tunnel**（ルーターは触らない） |
| 外向きの住所 | `osanpo.ortiz-ai.partners` → ミニPCの5072 |
| ドメイン管理 | Xserverドメイン（Xserverアカウント→ドメイン→⋮→ネームサーバー設定） |
| DNS移行 | 19件中13件を写す、NS5件は写さない、`*` ワイルドカードを忘れない。詳細は domain-and-relay.md |
| 移行する時間帯 | メールを使わない夕方以降、まとまった時間が取れる日 |
| 将来 | `office.ortiz-ai.partners` → 3Dオフィス(3939)。StackChan の ws 8765 / 写真 8766 も同じトンネルで外へ（本体の Fallback Gateway URL に入れる） |

## 2026-09-17 ミニPCで通ったこと

- Python 3.12 / Git / Node 24 / Claude Code 2.1.273 を winget と npm で導入（`--source winget`、`--allow-scripts` が必要だった）
- `C:\osanpo` にリポジトリを置き、`start-osanpo.bat` でサーバ起動、閲覧ページ表示
- 手で渡した写真（ボールプール）に、おさんぽの子が最初の一言を返した。**ミニPCの中で「写真→一言→日記」が一周**
- 直した詰まり: .bat の日本語コメント、PowerShell の実行ポリシー、npm の allow-scripts、claude.cmd 経由の呼び出し、日本語の文字化け

## まだ決めていない・確認待ち

- StackChanの出荷時ファームと初期画面（開封後）
- モバイルバッテリーのPD出力表記
- 700mAhでの実稼働時間（実測）
- diary.jsonl のバックアップ先
- stackchan-mcp のファームが手元の個体で動くか（実機待ち）
- `personas/ortiz.md` の中身（ゆうころが書く）
- 別のスタックチャンに載っているヴェルティは何で動いているか（ヴェルティ本人はこのセッションでは知らない）
