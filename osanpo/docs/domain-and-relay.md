...
前提: ドメインは **Xserverドメイン** で管理（Xserverアカウント→ドメイン一覧に表示。利用期限 2028/05/15、自動更新ON。2026-09-16確認）。サイトとメールはXserverで稼働中。
...
1. Xserverサーバーパネル →「DNSレコード設定」→ `ortiz-ai.partners` の全レコードを控える（スクショ＋テキスト）
   ...
   - CNAME など
2. Xserverアカウント →「ドメイン」→ `ortiz-ai.partners` の右端「⋮」→「ネームサーバー設定」が開けることを確認（確認済み: ここで変更する）
...
1. Xserverアカウント →「ドメイン」→ `ortiz-ai.partners` →「ネームサーバー設定」→「その他のネームサーバーを使用」→ Cloudflareの2つに変更
